...
表里那一轮之后，同一个 0.6B 又做了 120 步监督训练，接着 24 步 RLCD、48 次优化更新。按事先定好的开发集规则，留下的是 RLCD 第 12 步。这是实验预览。表里的权重没有换。

留出的 MBPP 完成判断从 542/1048（51.72%）到 606/1048（57.82%）。同一题里，正确程序排在错误程序前面的配对从 305/524 到 348/524。起点另有 3 对同分，后来另有 5 对同分，都没有计进前面的分数。AUROC 从 0.5156 到 0.5895。负例被放行从 169/524（32.25%）到 221/524（42.18%）。

工具前置条件题保持 16/16。新增业务策略题从 75/80 到 80/80。这两组都是小规模合成规则题。

旧检查没有跟着涨。编码边界题从 18/36 到 14/36。真实补丁回归停在 255/512，成功补丁的召回仍是 0。四个旧工作流上和教师最大概率项的一致率是：客服 84.0%，发票 87.2%，安全事件 78.4%，Agent 轨迹 73.4%。这几行和上面各 500 题的表不是同一组数字。

进入梯度的是 MBPP 的训练划分。官方验证划分和测试划分没有进入。训练代码在 [`typed_decisions/agent_completion_v9`](typed_decisions/agent_completion_v9)。
